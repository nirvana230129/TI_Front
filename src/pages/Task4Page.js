...
class Task4Page extends React.Component {
    ...
    handleSubmit(event) {
        ...
        if (this.state.c_is_generated) {
            ...
            axios.post(this.props.baseUrl + this.path, { a, b, c })
                .then(response => {
                    this.setState({
    ...
    generateRandomC() {
        ...
        const right = 70;
        this.setState({
            c: Math.floor(Math.random() * (right - left + 1)) + left,
            c_is_generated: true,
        });
    }
    ...
    render() {
        return (
            <div>
                <TaskDescription
                    taskNumber='4'
                    inputDescription='Введите число a — длину строки, число b — число дополнительных символов, значение числа c — вероятности помехи в каждом отдельном символе — выбирается случайно.'
                ...
                <form onSubmit={this.handleSubmit}>
                    ...
                    <NumberInput
                        ...
                        min='1'
                    />
                    <button type='button' onClick={this.generateRandomC}>
                        Сгенерировать новое значение для переменной c
                    </button>
                    <SubmitButton />
                </form>
                    ...
                    this.state.c_is_generated && this.state.button_is_pressed && (
                        <ResultContainer
                            result={
                                <>
                                    <ResultString
                                        string={this.state.is_decrypted ? 'Удалось декодировать ✅' : 'Не удалось декодировать ❌'}
                                    />
                                    <Table
